...
$query_Recinfo = "SELECT * FROM cpai where cp_hao='$key' ORDER BY id DESC";
...
mysql_select_db($database_connch21, $connch21);
$query_Recinfo = "SELECT * FROM cpai ORDER BY id DESC";
$query_limit_Recinfo = sprintf("%s LIMIT %d, %d", $query_Recinfo, $startRow_Recinfo, $maxRows_Recinfo);
$Recinfo = mysql_query($query_limit_Recinfo, $connch21) or die(mysql_error());
$row_Recinfo = mysql_fetch_assoc($Recinfo);
    ...
    <td ><table  width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#ACACAC">
        <tr valign="top"> 
          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr> 
                <td><strong><font color="#FF0000" size="3">车牌信息号码管理</font></strong>
                  <hr size="1" noshade> 
                  <table width="100%" border="0" cellspacing="1" cellpadding="2">
                    <tr bgcolor="#CCCCCC">
                      <td colspan="11" bgcolor="#FFFFFF" ><form name="form1" method="get" action="">
                   ...
                   <?php if(!empty($key)) {?> <input type="submit" name="fhui" id="fhui" value="返回"><?php } ?>
                      </form></td><td colspan="2" align="center"><span style=" color:#F00;">*&nbsp;</span>车牌号批量上传 </td><td colspan="2"  bgcolor="#FFFFFF"><form name="form2" method="post" enctype="multipart/form-data" action="plsc2017_cp/upload_excel.php" onSubmit="  jc();">
...
<tr>
  ...
  <input type="file" name="inputExcel">
    ...
    <input type="button" value="批量导出数据" onClick="javascript:location.href='./plsc2017_cp/dc/';">
   </td>
</tr>
</table>
</form></td>
...
function chek(){
	...
	return false;
					 ...
					 <form  name="form3"action="del_plsc.php" method="post">  <input name="tab" type="hidden" value="cpai">   
                    ...
                    <?php do { ?>
                      <tr bgcolor="#FFFFFF" style="cursor: hand;" onMouseOver="this.style.backgroundColor='#F0F0F0'" onMouseOut="this.style.backgroundColor=''">
                        ...
                        <td ><?php echo $row_Recinfo['cp_hao'];
                      ...
                      <!--  <td><?php  if($row_Recinfo['m1']=="-1"){echo "无线 | " ;} ?> <?php  if($row_Recinfo['m2']=="1"){echo "有线 |"; }?> <?php if($row_Recinfo['m3']=="2"){echo "商务一号通 | "; }?>
						...
						*/
                        ...
                        <td width="5%" align="center"><font size="2"><a href="cp_fix.php?<?php echo $MM_keepNone.(($MM_keepNone!="")?"&":"")."id=".$row_Recinfo['id'] ?>">编辑</a> —<a href="cpDel.php?<?php echo $MM_keepNone.(($MM_keepNone!="")?"&":"")."id=".$row_Recinfo['id'] ?>">删除</a></font></td>
                      </tr>
