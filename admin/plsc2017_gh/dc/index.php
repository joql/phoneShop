...
// mysql_select_db("db_39",$conn);               //连接数据库
 ...
 $sql = mysql_query("select * from kuhua");    //查询sql语句
/*--------------设置表头信息------------------*/
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', '无线m1')
            ->setCellValue('B1', '有线m2')
            ->setCellValue('C1', '商务一号通m3')
            ->setCellValue('D1', '省份1为黑龙江')
            ->setCellValue('E1', '城市')
            ->setCellValue('F1', '规率')
			->setCellValue('G1', '价格范围')
			->setCellValue('H1', '固话号码')
			->setCellValue('I1', '置底靓号vs天价靓号')
			->setCellValue('J1', '售价')
			->setCellValue('K1', '含费')
			->setCellValue('L1', '备注信息')
			->setCellValue('M1', '特殊字符')
	  ...
	  $rm3 = iconv("gbk", "UTF-8//IGNORE",$rs[14]);
	   ...
	   $objPHPExcel->setActiveSheetIndex(0)
           ...
           //  ->setCellValue("B".$i, $rm)  //由于我的这一列是中文，所以在上面进行了编码
             ...
             ->setCellValue("J".$i, $rs[10])
             ->setCellValue("K".$i, $rs[11])  
			 ->setCellValue("L".$i, $rm1)
			 ->setCellValue("M".$i, $rm2)
             ->setCellValue("N".$i, $rm3)
             ->setCellValue("O".$i, $rm4)
