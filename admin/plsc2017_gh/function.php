    ...
    if($result) //如果上传文件成功，就执行导入excel操作
    {
	   ...
	   $highestColumn = $sheet->getHighestColumn(); // 取得总列数
		...
		{ 
			...
			$sql = "INSERT INTO `kuhua`( `m1`, `m2`, `m3`, `bigtype`, `smalltype`, `grade`, `price`, `gh_hao`, `info_top`, `s_price`, `hfei`, `message`, `tszf`, `phpoto`, `info_editor`, `l_tel`, `wx`) VALUES('".$strs[0]."','".$strs[1]."','".$strs[2]."','".$strs[3]."','".$strs[4]."','".$strs[5]."','".$strs[6]."','".$strs[7]."','".$strs[8]."','".$strs[9]."','".$strs[10]."','".$strs[11]."','".$strs[12]."','".$strs[13]."','".$strs[14]."','".$strs[15]."','".$strs[16]."')";  //成品2  注：数据库，sj的字段除了id外，皆设为varchar 字符串型，并且除id为皆要设为允许为空，这么做为防止用户上传表格数据时哪个栏没写为空，而数据库要求不为空而出现错误
      ...
      // $msg = "导入成功！";
	 echo "<script language='javascript'>alert('固话号批量上传成功!');location.href='../gh_Admin.php';</script>";
    }else{
      // $msg = "导入失败！";
	  
	  echo "<script language='javascript'>alert('固话号批量上传失败!');location.href='../gh_Admin.php';</script>";
    }
   // return $msg;
}
?>