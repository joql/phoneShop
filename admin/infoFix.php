...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE info SET info_time=%s, classname=%s, smallclassname=%s, xx_smallclass=%s, info_title=%s, info_editor=%s, info_photo=%s, info_top=%s, info_flash=%s, info_content=%s, cp_je=%s, px=%s WHERE info_id=%s",
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#EEEEEE">
        <tr valign="top"> 
          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr> 
                <td><font color="#FF0000" size="3"><strong>修改服务范围信息</strong></font> 
                    ...
                    <tr> 
                      <td><form action="<?php echo $editFormAction; ?>"   method="POST" name="form1" style="margin=0px;">
                          <table  width="100%" border="0" cellspacing="1" cellpadding="0">
                            <tr> 
                              <td width="100px" align="center" bgcolor="#999999"><strong><font color="#FFFFFF" size="2">日期：</font></strong></td>
...
<td align="right">信息中类:</td>
                              <td><select name="smallclassname">
                                <?php
do {  
?>
...
?>
                              ...
                              <td align="right"><!--信息小类:--></td>
                              <td><!--<select name="xx_smallclass">
                                <?php
do {  
?>
                                <option value="<?php echo $row_Recordset1['x_mame']; ?>"<?php if (!(strcmp($row_Recordset1['x_mame'], $row_Recinfo['xx_smallclass']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Recordset1['x_mame']?></option>
                                <?php
...
do {  
?>
                                <option value="<?php echo $row_Rec_zhxg['zx_x_mame']; ?>" <?php if (!(strcmp($row_Rec_zhxg['zx_x_mame'], $row_Recinfo['zx_name']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Rec_zhxg['zx_x_mame']; ?></option>
                                <?php
} while ($row_Rec_zhxg = mysql_fetch_assoc($Rec_zhxg));
  $rows = mysql_num_rows($Rec_zhxg);
  if($rows > 0) {
      mysql_data_seek($Rec_zhxg, 0);
	  $row_Rec_zhxg = mysql_fetch_assoc($Rec_zhxg);
  }
?>
                              </select>-->
                              
                              <!--<input name="px" type="text" id="px" value="<?php echo $row_Recinfo['px']; ?>" size="5">-->
                              </td>
                            </tr>                            
                            <!--<tr> 
...
<td width="112" align="right" valign="top">是否幻灯:</td>
                              <td width="102" valign="top"> <input <?php if (!(strcmp($row_Recinfo['info_flash'],"1"))) {echo "checked=\"checked\"";} ?> name="info_flash" type="radio" value="1" >
                                是
