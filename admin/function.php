    ...
    if($result) //如果上传文件成功，就执行导入excel操作
    {
	   ...
	   $highestColumn = $sheet->getHighestColumn(); // 取得总列数
		...
		{ 
			...
			$sql = "INSERT INTO sj(pid,tel,theme,price,day,sj_hao,info_top,s_price,hfei,message,tszf,info_editor,l_tel,wx,jlou) VALUES('".$strs[0]."','".$strs[1]."','".$strs[2]."','".$strs[3]."','".$strs[4]."','".$strs[5]."','".$strs[6]."','".$strs[7]."','".$strs[8]."','".$strs[9]."','".$strs[10]."','".$strs[11]."','".$strs[12]."','".$strs[13]."','".$strs[14]."')";  //成品2  
	 ...
	 echo "<script language='javascript'>alert('手机号批量上传成功!');location.href='../sjAdmin.php';</script>";
	  ...
	  echo "<script language='javascript'>alert('手机号批量上传失败!');location.href='inddex.php';</script>";
    }
   // return $msg;
}
?>