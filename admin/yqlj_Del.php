...
$colname_Rec_yl = "-1";
if (isset($_GET['id'])) {
  $colname_Rec_yl = $_GET['id'];
}
mysql_select_db($database_connch21, $connch21);
$query_Rec_yl = sprintf("SELECT * FROM yqlj WHERE id = %s", GetSQLValueString($colname_Rec_yl, "int"));
$Rec_yl = mysql_query($query_Rec_yl, $connch21) or die(mysql_error());
$row_Rec_yl = mysql_fetch_assoc($Rec_yl);
$totalRows_Rec_yl = mysql_num_rows($Rec_yl);
?>
<html>
<head>
<title>广告管理</title>
...
<hr size="1" noshade> 
                  <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr> 
                      <td valign="top"><font size="2"><strong>日期</strong><?php echo $row_Rec_yl['time']; ?>：<strong> </font></td>
                    </tr>
                  </table>
                </td>
              </tr>
              <tr> 
                <td><form name="form1" method="get" action="">
                  ...
                  <?php echo $row_Rec_yl['name']; ?>
                  <input name="delsure" type="hidden" id="delsure" value="1">
                  <input name="id" type="hidden" id="id" value="<?php echo $row_Rec_yl['id']; ?>">
                </form></td>
              </tr>
            </table></td>
