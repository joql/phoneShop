...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE zx_xx_smallclass SET zx_x_mame=%s, class_id=%s, smallclass_id=%s, xx_name_id=%s, x_num=%s WHERE zh_id=%s",
                       GetSQLValueString($_POST['zx_x_mame'], "text"),
                       GetSQLValueString($_POST['class_id'], "text"),
                       GetSQLValueString($_POST['small_id'], "text"),
                       GetSQLValueString($_POST['xx_name_id'], "text"),
                       GetSQLValueString($_POST['smallclassnum'], "int"),
                       GetSQLValueString($_POST['zh_id'], "int"));

  mysql_select_db($database_connch21, $connch21);
  $Result1 = mysql_query($updateSQL, $connch21) or die(mysql_error());

  $updateGoTo = "zx_smallclassAdmin_xx.php";
  if (isset($_SERVER['QUERY_STRING'])) {
    $updateGoTo .= (strpos($updateGoTo, '?')) ? "&" : "?";
    $updateGoTo .= $_SERVER['QUERY_STRING'];
  }
...
mysql_select_db($database_connch21, $connch21);
$query_Rec_xx = "SELECT * FROM xx_smallclass";
...
$colname_Rec_zhfl = "-1";
if (isset($_GET['zh_id'])) {
  $colname_Rec_zhfl = $_GET['zh_id'];
}
mysql_select_db($database_connch21, $connch21);
$query_Rec_zhfl = sprintf("SELECT * FROM zx_xx_smallclass WHERE zh_id = %s", GetSQLValueString($colname_Rec_zhfl, "int"));
$Rec_zhfl = mysql_query($query_Rec_zhfl, $connch21) or die(mysql_error());
$row_Rec_zhfl = mysql_fetch_assoc($Rec_zhfl);
$totalRows_Rec_zhfl = mysql_num_rows($Rec_zhfl);
?>
<html>
<head>
<title>广告管理</title>
    ...
    <td><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#EEEEEE">
        <tr valign="top"> 
          ...
          <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr> 
                <td height="192"><font color="#FF0000" size="3"><strong>修改最小类别</strong>&nbsp;(D类）</font> 
                          ...
                          <table width="100%" border="0" cellspacing="1" cellpadding="2">
                            ...
                            <td width="80" align="right" bgcolor="#999999"><strong><font color="#FFFFFF" size="2">信息类别：</font></strong></td>
                              <td valign="top"><font size="2"> 
                                <input name="zx_x_mame" type="text" id="zx_x_mame" value="<?php echo $row_Rec_zhfl['zx_x_mame']; ?>">
                                </font></td>
                            </tr>
...
do {  
?>
                                   <option value="<?php echo $row_Recbigclass['classname']?>"<?php if (!(strcmp($row_Recbigclass['classname'], $row_Rec_zhfl['class_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Recbigclass['classname']?></option>
                                   <?php
...
$row_Rec_small['smallclassname']; ?>"<?php if (!(strcmp

($row_Rec_small['smallclassname'], $row_Rec_zhfl['smallclass_id']))) {echo 

"selected=\"selected\"";} ?>><?php echo $row_Rec_small

['smallclassname']?></option>
                                   <?php
...
?>
                            ...
                            <tr> 
                              <td align="right" valign="top" bgcolor="#999999"><strong><font color="#FFFFFF" size="2">所属小类：<br>
                                </font></strong></td>
                              <td valign="top"> <font size="2">
                                <select  name="xx_name_id" type="text" id="xx_name_id">
                                  <?php
do {  
?>
                                  <option value="<?php echo $row_Rec_xx['x_mame']; ?>" <?php if (!(strcmp($row_Rec_xx['x_mame'], $row_Rec_zhfl['xx_name_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Rec_xx['x_mame']; ?></option>
                                  <?php
} while ($row_Rec_xx = mysql_fetch_assoc($Rec_xx));
  $rows = mysql_num_rows($Rec_xx);
  if($rows > 0) {
      mysql_data_seek($Rec_xx, 0);
	  $row_Rec_xx = mysql_fetch_assoc($Rec_xx);
  }
?>
                                  <?php
do {  
?>
                                  <option value="<?php echo $row_Rec_xx['smallclass_id']?>"<?php if (!(strcmp($row_Rec_xx['smallclass_id'], $row_Rec_zhfl['xx_name_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_Rec_xx['smallclass_id']?></option>
                                  <?php
} while ($row_Rec_xx = mysql_fetch_assoc($Rec_xx));
  $rows = mysql_num_rows($Rec_xx);
  if($rows > 0) {
...
?>
                           ...
                           <tr> 
                              ...
                              <td valign="top"> <font size="2">
                                 <input name="smallclassnum" type="text" id="smallclassnum" value="<?php echo $row_Rec_zhfl['x_num']; ?>">
                                <input name="zh_id" type="hidden" id="zh_id" value="<?php echo $row_Rec_zhfl['zh_id']; ?>">
                              </font></td>
                            </tr>  
